...
ggplot(dplyr::filter(plot_pid_tab, pid_lean %in% c("Dem", "Rep"))) +
    aes(x = cycle,  y = prop) +
    facet_wrap(~ gender,
...
ggplot(dplyr::filter(plot_turnout_gender, voted == 1)) +
    aes(x = cycle, y = prop) +
...
gap_frame = anes |>
    ...
    tidyr::pivot_wider(names_from = "gender", values_from = dem_share:n_cycle) |>
    group_by_all() |>
    mutate(
