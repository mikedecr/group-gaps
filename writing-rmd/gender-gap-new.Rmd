---
title: |
  | Partisanship, Mobilization, and Persuasion in Group Voting: A Study of the Gender Gap\thanks{
      We thank Luis Fraga, Devin Judge-Lord, Ryan Moore, Byron Shafer, Marcy Shieh, Paul Testa, and participants at the American Politics Workshop at the University of Wisconsin-Madison, the 2016 Toronto Political Behavior Workshop, and the 2017 meeting of the Midwest Political Science Association for comments on earlier drafts of this paper. Authors are listed alphabetically.
    }
author: |
  | Barry C. Burden\thanks{
      Corresponding author. 
      Professor of Political Science, University of Wisconsin--Madison.
      \href{mailto:bcburden@wisc.edu}{\texttt{bcburden@wisc.edu}}
    } 
  | Michael G. DeCrescenzo\thanks{
      Ph.D. Candidate, Political Science, University of Wisconsin--Madison.
      \href{mailto:decrescenzo@wisc.edu}{\texttt{decrescenzo@wisc.edu}}
    }
date: |
  | This version: `r format(Sys.time(), '%B %d, %Y')`
...
abstract: |
  ...
  Pundits and researchers often interpret election outcomes in light of these group differences in voting, but many analyses base their conclusions on the behavior of voters only. 
  Conditioning all analysis on voters who turn out will neglect important factors such as group size and group turnout rates.
  We present a theoretical framework to understand group differences in voting by expressing party vote shares as the result of partisan predispositions in the electorate that are transformed through partisan mobilization, partisan defection, and the choices of unaffiliated voters.
  We apply this method to the gender gap in U.S. presidential voting.
  Many analysts imply that a larger gender gap favors Democratic candidates, but we show that the size of the gender gap has no necessary bearing on the partisan vote outcome.
  Both the gender gap and the Democratic vote in presidential elections have both increased in the past several decades, but this relationship is spurious.
  Our method clarifies how the primary cause of the gender gap (partisan change) was actually harmful to the Democrats.
  Meanwhile, forces that increased the Democratic vote (mobilization and persuasion) were only minor influences on the gender gap.
compact-title: false
fontsize: 12pt
geometry: margin = 1in
indent: yes
...
In an electoral democracy
  \todo{redundant?}
such as the United States, parties and candidates win control of government by assembling coalitions of social groups to form a majority of voters. Retrospections on specific elections commonly include casual observations that attribute the outcome to such things as "strong backing from the Hispanic community" or "lower-than-expected turnout among rural voters." These observations unwittingly blur a key feature of any electoral coalition: the coalition is a compromise. If a party has an electoral advantage among a social group's members, it may have a disadvantage among nonmembers. When examined in isolation, a political party may appear to "benefit" from the voting bloc only if the votes gained from group members outweigh the votes lost among nonmembers. A large advantage among a small group may be less valuable to a party than a small advantage among a large group. A political party forms a winning coalition if it retains a vote advantage after summing over the various cross-cutting cleavages that divide an electorate. 

Recent U.S.\ electoral history confirms that political parties strategize about their electoral coalitions in this way, as a balancing act of votes gained and votes lost across social cleavages. For instance, analysis of the 2016 presidential election identified the shift among "White working class" voters in Midwestern states as a primary driver of Republican candidate Donald Trump's victory over Democratic candidate Hillary Clinton. As a result, Democratic strategists have debated whether they should rebuild their coalition by reclaiming votes from rural Whites (at the risk on demobilizing urbanites and people of color) or by reengaging the more diverse "Obama coalition"
  \todo{cite} 
(at the risk of further losses among rural Whites). Republican strategists, meanwhile, must balance their advantage among White votes against their disadvantage among voters of color.\footnote{
  In the aftermath of the 2012 election, Republican campaign strategist Steve Schmidt commented about the Republican Party's support among White voters by noting, "Twenty-four years ago, if you got 60 percent of the White vote, you got 404 Electoral College votes, and you won in a landslide. [In 2012], 60 percent of the White vote yields you a pretty convincing Electoral College defeat." Post Mortem on the Election, Stanford University, \url{https://web.archive.org/web/20190729212143/https://www.youtube.com/watch?v=6fTsy5FRCSY&gl=US&hl=en&has_verified=1&bpctr=9999999999}
}
Parties make calculations of this sort across numerous social cleavages, including Whites and Nonwhites, urban and rural voters, men and women, and the intersections among these and other groups.
  \todo{cites}
We ask two related questions about group-level voting and electoral coalitions. As the "gap" in vote choice between two social groups grows wider, under which conditions will either party benefit numerically? And how does the composition of a voting gap (differences in baseline partisanship, mobilization, or swing voting) determine whether a shift in a voting gap will benefit one party or the other?

We develop a general theory and an empirical approach to describe how voting gaps between groups in the electorate can advantage one party over another. Our approach can be applied to various group cleavages such as racial and ethnic groups, college and non-college educated citizens, the working and non-working classes, and urban and rural residents. As political parties compete to build winning coalitions, they attract certain voters while potentially repelling other voters, adding to or subtracting from a party's overall vote share in the process. Our approach measures how much of a party's total vote is attributable to partisan predispositions, the mobilization of same-party supporters, swing voting by other-party supporters, and the votes of the party-unaffiliated, tracing gender differences in each of these mechanisms. By decomposing party votes into these building blocks, we show that voting gaps and election results can be expressed as functions of the same underlying processes. We can directly compare the electoral impact of partisanship, mobilization, and persuasion on the same scale, which existing methods cannot accomplish. The mathematical basis of the approach requires only simple arithmetic, but the model can expanded to incorporate more complex statistical considerations.

We apply our method to the case of the gender gap in U.S.\ voting, which has grown to double digits in recent presidential elections. The gender gap, conventionally defined as the percent of women voting Democratic minus the percent of men voting Democratic, can fluctuate for several reasons. Women may vote more highly Democratic, but men may also vote more highly Republican. Because the gap is measured as a difference in proportions, and because proportions do not convey the absolute number of votes at stake, the size of the gender gap does not imply which party benefits overall from an increasing or decreasing gender gap. The votes of men and women are not as divergent as other notable groups in the electorate, but because men and women are nearly equally-sized groups, small shifts in their aggregate behavior have acute electoral effects. This has left journalists, pundits, and academics divided about the gender gap's implications for the parties' electoral strategies. Recent political commentary often attributes the gender gap to the Democratic Party's "strength" among women voters, but foundational research on the gender gap traces the emergence of the gap primarily to an increase in Republican party identification among men [e.g.\ @kaufmann1999changing; @box2004dynamics]. Recent decades have also seen a decisive increase in turnout among women voters, further confounding the relationship between the size of the gender gap and the Democratic margin.
  \todo{cite}
We contend that the effect of the gender gap on the Democratic vote is unclear without a model to describe the processes that create the gap. We use our approach to decompose the gap into its partisan, mobilization, and persuasion components to determine which processes yield advantages for which party over time.
...
The gender gap, conventionally defined as the percent of women voting Democratic minus the percent of men voting Democratic, can fluctuate for several reasons. Women may vote more highly Democratic, but men may also vote more highly Republican. 
...
This is the focus of our second question. To the extent that the gender gap and the Democratic vote are linked, what balance of long and short-term mechanisms shape electoral outcomes? 
-->

Our approach synthesizes classic findings from the academic literature with new insights about the relationship between the gender gap and election outcomes. With survey data from `r anes %$% n_distinct(cycle)` presidential elections (`r min(anes$cycle)`--`r max(anes$cycle)`), we show that the forces driving over-time variation in the gender gap are not the same forces that drive over-time variation in the Democratic vote. Our method recovers the finding from the gender and voting literature that the gender gap initially emerged from a long-term increase in Republican partisanship among men from the 1960s through the 1980s, which negatively affected the Democratic vote. During roughly the same time period, the Democratic Party's historical disadvantages in partisan mobilization and partisan defections narrowed, boosting the Democratic vote share but in a relatively gender-neutral way. Our method shows that short-term fluctuations in these mechanisms largely cancel each other out in the aggregate, which explains why the relationship between the gender gap and election outcomes has been difficult to characterize.
  \todo{dynamics?} 

<!-- Variance decomposition of the year-over-year change? -->
...
- We begin by showing, with actual examples, two things
  - an advantage within one group of is only an "absolute advantage" if the group advantage is not offset elsewhere
  - for a given size of the gender gap, there are a number of vote outcomes that can be rationalized
- The standard measurement approaches for these concepts do not help us out

Into should be reworked to get more motivation maybe?

- What's wrong w/
  - current standard measures?
  - election outcomes are sums of things, but we are interested in the components of the sums
  - Petrocik: "realignment" is social group coalitions. Coalitions change even if one party stays dominant
  - Axelrod (Zingher) for "contributions" of groups, but the contribution of a group 
...
\begin{comment}
  ...
  Consider an electorate with two parties, $A$ and $B$, who compete for the support of voters who belong to social group $G$ and voters who are not members of $G$ (indicated by $\neg G$). Let $p$ be the vote share for party $A$ among social group $G$. In a scenario where $G$ is a key voting bloc for party $A$, $A$ will win a majority of voters from $G$ ($p > .5$). The question party $A$ faces going into an is, given that they can expect $p$ of the vote from $G$, how many more votes do they need to win a majority over party $B$?

```{r demo}
fake <- 
  tibble(G = c(.1, .3, .5)) %>%
  crossing(P = seq(0.5, 1, .01)) %>%
  mutate(
    min_q = 
      ( (-2 * G * P) + (2 * G) - 1) / 
      ( 2 * (G - 1) )
...
ggplot(data = fake) +
  aes(x = P,  y = min_q) +
  geom_line(aes(color = as.factor(G))) +
  ylim(c(0, 1)) +
  xlim(c(0, 1))
  ```
...
```{r dot-net-votes, include = TRUE}
  include_graphics(here("writing-rmd", "paper-figs", "dot-net-votes.pdf"))
```
...
```{r dot-net-mechanisms, include = TRUE}
  include_graphics(here("writing-rmd", "paper-figs", "dot-net-vote-mechanisms.pdf"))
```
...
```{r dot-net-gap, include = TRUE}
  include_graphics(here("writing-rmd", "paper-figs", "dot-net-gap-mechanisms.pdf"))
```
